/* ================================================================
   ...
   Shared logic for index.html (customer) and admin.html (admin)
   ================================================================ */
...
// ----------------------------------------------------------------
// SECTION NAVIGATION (customer page)
// ----------------------------------------------------------------
...
function handleBusinessType() {
  ...
  toggle('otherBusinessField', val === 'Other');
}
...
function handleExistingSocial() {
  ...
  // Reset the opposite branch
  if (val === 'yes') {
    document.querySelectorAll('input[name="createProfiles"]').forEach(r => r.checked = false);
    document.querySelectorAll('input[name="wantMaintenance"]').forEach(r => r.checked = false);
    toggle('wantMaintenanceField', false);
  } else {
    document.querySelectorAll('input[name="maintainExisting"]').forEach(r => r.checked = false);
    document.querySelectorAll('input[name="maintainPlatform"]').forEach(c => c.checked = false);
...
// PRICING
// ----------------------------------------------------------------
const PRICING_KEY = 'ds_adminPricing';
...
function updatePricing() {
  ...
  if (pages > 0) {
    ...
    if (pages > 1) {
      const extra = pages - 1;
      const extraCost = extra * pricing.perPage;
      items.push({ label: `Additional ${extra} page(s) × ${formatLKR(pricing.perPage)}`, amount: extraCost });
      total += extraCost;
    }
  }
  ...
  const needsMaintenance = maintainExisting === 'yes' || wantMaintenance === 'yes';
  const pkgVal = document.getElementById('smPackage')?.value;

  if (pkgVal && needsMaintenance) {
  ...
  if (items.length === 0) {
    itemsEl.innerHTML = `<div class="pricing-item"><span>Select options above to see pricing</span><span>—</span></div>`;
  } else {
    itemsEl.innerHTML = items.map(item =>
      `<div class="pricing-item"><span>${item.label}</span><span>${formatLKR(item.amount)}</span></div>`
    ).join('');
  }
  ...
  // Package hint
  updatePackageHint(pkgVal, pricing, needsMaintenance);
}

function updatePackageHint(pkgVal, pricing, needsMaintenance) {
  const el = document.getElementById('packageHint');
  if (!el) return;
  if (!pkgVal) { el.textContent = ''; return; }
  ...
  const applied = needsMaintenance ? ' — applied to total' : ' — select maintenance above to apply';
...
function validateWebsiteForm() {
  ...
  ok = validateField('websiteRequirement', 'websiteRequirementError', el => el.value.trim().length >= 10, 'Please describe your requirements (min 10 characters).') && ok;
  ...
  ok = validateField('contactEmail', 'contactEmailError', el => validateEmail(el.value.trim()), 'Please enter a valid email address.') && ok;
...
function validateBusinessForm() {
  ...
  ok = validateField('bpEmail', 'bpEmailError', el => validateEmail(el.value.trim()), 'Please enter a valid email.') && ok;
  return ok;
}

// ----------------------------------------------------------------
// COLLECT & CALCULATE TOTAL (reusable)
// ----------------------------------------------------------------
function calcTotal(pricing, pages, createProfiles, maintainExisting, wantMaintenance, pkgVal) {
  let total = 0;
  ...
  if (pkgVal && needsMaint) total += pricing[`package${pkgVal}`] || 0;
  return total;
}
...
function collectWebsiteData() {
  ...
  const wantMaintenance = document.querySelector('input[name="wantMaintenance"]:checked')?.value || '';
  const pkgVal = document.getElementById('smPackage').value;
  const total = calcTotal(pricing, pages, createProfiles, maintainExisting, wantMaintenance, pkgVal);
  ...
  return {
    ...
    type: 'website',
    businessType: businessType === 'Other' && otherBusiness ? `Other (${otherBusiness})` : businessType,
...
// ----------------------------------------------------------------
const REQUESTS_KEY = 'ds_customerRequests';
...
async function handleWebsiteSubmit(e) {
  ...
  saveRequest(collectWebsiteData());
...
// ----------------------------------------------------------------
// FORM SUBMISSION — BUSINESS PROCESSES
// ----------------------------------------------------------------
...
function savePricing() {
  ...
  const pricing = {
    ...
    smCreation: get('priceSMCreation'),
...
function renderTable() {
  ...
  tbody.innerHTML = pageItems.map((req, i) => {
    ...
    return `<tr>
      ...
      <td>${req.numPages || '—'}</td>
      <td style="max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${esc(platforms)}">
        ${esc(platforms)}
      </td>
...
function renderPagination(total, perPage, totalPages, start) {
  ...
  infoEl.textContent = `Showing ${start + 1}–${end} of ${total} request${total !== 1 ? 's' : ''}`;
...
function updateStats(requests) {
  ...
  setEl('statTotal', requests.length);

  const revenue = requests.reduce((s, r) => s + (r.totalPrice || 0), 0);
  setEl('statRevenue', formatLKR(revenue));

  const withSocial = requests.filter(r =>
    (r.socialPlatforms?.length > 0) || r.socialIntegration === 'yes'
  ).length;
  setEl('statSocial', withSocial);

  const webReqs = requests.filter(r => r.numPages && parseInt(r.numPages));
  const avg = webReqs.length
    ? (webReqs.reduce((s, r) => s + (parseInt(r.numPages) || 0), 0) / webReqs.length).toFixed(1)
    : '0';
  setEl('statAvgPages', avg);
}
...
function exportJSON() {
  const requests = getRequests();
  if (requests.length === 0) { showToast('No requests to export.', 'ℹ️'); return; }
